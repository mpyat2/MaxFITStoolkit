...
procedure CFAtoRGB(var FITSfile: FITSRecordFile; const FITSFileName: string; const OutputDir: string; const Prefix: string; const BayerPattern: TBayerPattern; Overwrite: Boolean);
var
  N: Integer;
...
procedure ProcessFile(const FileName: string; const OutputDir: string; const Prefix: string; const BayerPattern: TBayerPattern; Overwrite: Boolean);
var
...
begin
  ...
  try
    ...
    CFAtoRGB(FITSfile, FileName, OutputDir, Prefix, BayerPattern, Overwrite);
  finally
    CloseFile(FITSfile);
  end;
  WriteLn(': done.');
...
procedure ProcessInput(const FileMasks: array of string; const OutputDir: string; const Prefix: string; const BayerPattern: TBayerPattern; Overwrite: Boolean);
var
  I, N, Ntotal: Integer;
begin
  try
    ...
    for N := 0 to Length(FileMasks) - 1 do begin
      ...
      for I := 0 to FileList.Count - 1 do begin
        ProcessFile(FileList[I], OutputDir, Prefix, BayerPattern, Overwrite);
        Inc(Ntotal);
      end;
    end;
...
begin
  ...
  try
    ProcessInput(InputFileMasks, OutputDir, Prefix, BayerPattern, Overwrite);
  finally
    FreeAndNil(FileList);
  end;
